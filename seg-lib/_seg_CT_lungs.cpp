...
*/
    ...
    int iter=1;
    while (out) {
        calcE_mask(T1,MRF,Expec,&loglik,BiasField,S2L,M,V,CurrSizes,segment_param->verbose_level);
        //Maximization
        calcM_mask(T1,Expec,BiasField,S2L,M,V,NULL,NULL,CurrSizes,segment_param->verbose_level);
...
nifti_image * Copy_ShortExpec_to_Result_Lungs(PrecisionTYPE * Expec,
    ...
    for(int i=0; i<CurrSizes->numelmasked; i++,Short_2_Long_Indices_PRT++){
        ...
        Resultdata_class[*Short_2_Long_Indices_PRT]=(exp(BiasField[i]*0.693147181)-1)*(CurrSizes->rescale_max[0]-CurrSizes->rescale_min[0])+CurrSizes->rescale_min[0];
    }
