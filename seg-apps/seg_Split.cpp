...
void Usage(char *exec)
{
    ...
    printf("\t* * Options * *\n");
    printf("\t-mode <int>\t\tOutput Mode [0 = Bias Corrected Image, 1 = Cortical GM, 2 = Brain] (default = 0)\n");
    printf("* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *\n");
    return;
}
...
int main(int argc, char **argv)
{
    ...
    if(mode==2){
        for(int i=0; i<Result->nvox; i++){
        Result_PTR[i]=Segmentation_PTR[i+(WMclass+1)*Result->nvox]+Segmentation_PTR[i+(GMclass+1)*Result->nvox]+Segmentation_PTR[i+(dGMclass+1)*Result->nvox];
        }
